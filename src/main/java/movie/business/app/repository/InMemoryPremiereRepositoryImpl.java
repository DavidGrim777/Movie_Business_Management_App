...
@Slf4j
public class InMemoryPremiereRepositoryImpl implements PremiereRepository {

    @Override
    public void save(Premiere premiere, boolean testMode) {
        Map<String, Premiere> currentData = loadPremiereFromFile();
        currentData.put(premiere.getId(), premiere);
        savePremieresToFile(currentData, testMode);
    }

    @Override
    public void deleteById(String id, boolean testMode) {
        Map<String, Premiere> currentData = loadPremiereFromFile();
        if (currentData.remove(id) != null) {
            savePremieresToFile(currentData, testMode);
            System.out.println("Премьера с ID " + id + " успешно удалена.");
        } else {
    ...
    @Override
    public Optional<Premiere> findById(String id) {
        Map<String, Premiere> premiereMap = loadPremiereFromFile();
        return Optional.ofNullable(premiereMap.get(id));
    }

    @Override
    public List<Premiere> findAll() {
        return new ArrayList<>(loadPremiereFromFile().values());
    }

    @Override
    public void saveAll(List<Premiere> premieres, boolean testMode) {
        Map<String, Premiere> premiereMap = new HashMap<>();
        for (Premiere premiere : premieres) {
            premiereMap.put(premiere.getId(), premiere);
        }
        savePremieresToFile(premiereMap, testMode);
    }

    @Override
    public void deleteAll(boolean testMode) {
        String fileName = testMode ? "test_premieres.txt" : "premieres.txt";
        try {
            Files.deleteIfExists(Paths.get(fileName));
            System.out.println("Файл премьер " + fileName + " удалён.");
            log.info("Файл премьер {} успешно удалён.", fileName);
        } catch (IOException e) {
